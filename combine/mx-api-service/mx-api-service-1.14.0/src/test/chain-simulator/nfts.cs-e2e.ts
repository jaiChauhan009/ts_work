...
describe('NFTs e2e tests with chain simulator', () => {
  describe('GET /nfts', () => {
    ...
    it('should return nfts filtered by different type combinations', async () => {
      // Test NonFungibleDCDT filter
      const nonFungibleResponse = await axios.get(`${config.apiServiceUrl}/nfts?type=${NftType.NonFungibleDCDT}`);
      expect(nonFungibleResponse.status).toBe(200);
      expect(nonFungibleResponse.data).toBeInstanceOf(Array);
      expect(nonFungibleResponse.data.every((nft: any) => nft.type === NftType.NonFungibleDCDT)).toBe(true);

      // Test SemiFungibleDCDT filter  
      const semiFungibleResponse = await axios.get(`${config.apiServiceUrl}/nfts?type=${NftType.SemiFungibleDCDT}`);
      expect(semiFungibleResponse.status).toBe(200);
      expect(semiFungibleResponse.data).toBeInstanceOf(Array);
      expect(semiFungibleResponse.data.every((nft: any) => nft.type === NftType.SemiFungibleDCDT)).toBe(true);

      // Test combined NonFungible and SemiFungible filter
      const combinedResponse = await axios.get(`${config.apiServiceUrl}/nfts?type=${NftType.NonFungibleDCDT},${NftType.SemiFungibleDCDT}`);
    ...
    it('should return nfts filtered by different subType combinations', async () => {
      // Test NonFungibleDCDTv2 filter
      const nonFungibleV2Response = await axios.get(`${config.apiServiceUrl}/nfts?subType=${NftSubType.NonFungibleDCDTv2}`);
      expect(nonFungibleV2Response.status).toBe(200);
      expect(nonFungibleV2Response.data).toBeInstanceOf(Array);
      expect(nonFungibleV2Response.data.every((nft: any) => nft.subType === NftSubType.NonFungibleDCDTv2)).toBe(true);

      // Test SemiFungibleDCDT filter
      const semiFungibleResponse = await axios.get(`${config.apiServiceUrl}/nfts?subType=${NftSubType.SemiFungibleDCDT}`);
      expect(semiFungibleResponse.status).toBe(200);
      expect(semiFungibleResponse.data).toBeInstanceOf(Array);
      expect(semiFungibleResponse.data.every((nft: any) => nft.subType === NftSubType.SemiFungibleDCDT)).toBe(true);

      // Test combined NonFungibleDCDTv2 and SemiFungibleDCDT filter
      const combinedResponse = await axios.get(`${config.apiServiceUrl}/nfts?subType=${NftSubType.NonFungibleDCDTv2},${NftSubType.SemiFungibleDCDT}`);
      expect(combinedResponse.status).toBe(200);
      expect(combinedResponse.data).toBeInstanceOf(Array);
      expect(combinedResponse.data.every((nft: any) =>
        [NftSubType.NonFungibleDCDTv2, NftSubType.SemiFungibleDCDT].includes(nft.subType)
      )).toBe(true);
    });
    ...
    it('should return nfts with owner field defined only for NonFungibleDCDT type', async () => {
      const response = await axios.get(`${config.apiServiceUrl}/nfts?withOwner=true&type=${NftType.NonFungibleDCDT}`);
      expect(response.status).toBe(200);
      expect(response.data).toBeInstanceOf(Array);
      expect(response.data.every((nft: any) => nft.type === 'NonFungibleDCDT')).toBe(true);
    ...
    it('should return nfts with supply field defined only for SemiFungibleDCDT type', async () => {
      const response = await axios.get(`${config.apiServiceUrl}/nfts?withSupply=true&type=${NftType.SemiFungibleDCDT}`);
      expect(response.status).toBe(200);
      expect(response.data).toBeInstanceOf(Array);
      expect(response.data.every((nft: any) => nft.type === 'SemiFungibleDCDT')).toBe(true);
  ...
  describe('GET /nfts/count', () => {
    ...
    it('should return the total number of nfts filtered by type', async () => {
      const nftsByTypeNFT = nfts.data.filter((nft: any) => nft.type === NftType.NonFungibleDCDT);
      const responseNFT = await axios.get(`${config.apiServiceUrl}/nfts/count?type=${NftType.NonFungibleDCDT}`);
      expect(responseNFT.status).toBe(200);
      expect(responseNFT.data).toStrictEqual(nftsByTypeNFT.length);

      const nftsByTypeSFT = nfts.data.filter((nft: any) => nft.type === NftType.SemiFungibleDCDT);
      const responseSFT = await axios.get(`${config.apiServiceUrl}/nfts/count?type=${NftType.SemiFungibleDCDT}`);
    ...
    it('should return the total number of nfts filtered by subType', async () => {
      const nftsBySubTypeNFT = nfts.data.filter((nft: any) => nft.subType === NftSubType.NonFungibleDCDTv2);
      const responseNFT = await axios.get(`${config.apiServiceUrl}/nfts/count?subType=${NftSubType.NonFungibleDCDTv2}`);
      expect(responseNFT.status).toBe(200);
      expect(responseNFT.data).toStrictEqual(nftsBySubTypeNFT.length);

      const nftsBySubTypeSFT = nfts.data.filter((nft: any) => nft.subType === NftSubType.SemiFungibleDCDT);
      const responseSFT = await axios.get(`${config.apiServiceUrl}/nfts/count?subType=${NftSubType.SemiFungibleDCDT}`);
      expect(responseSFT.status).toBe(200);
  ...
  describe('GET /nfts/c alternative', () => {
    ...
    it('should return the total number of nfts filtered by type', async () => {
      const nftsByTypeNFT = nfts.data.filter((nft: any) => nft.type === NftType.NonFungibleDCDT);
      const responseNFT = await axios.get(`${config.apiServiceUrl}/nfts/c?type=${NftType.NonFungibleDCDT}`);
      expect(responseNFT.status).toBe(200);
      expect(responseNFT.data).toStrictEqual(nftsByTypeNFT.length);

      const nftsByTypeSFT = nfts.data.filter((nft: any) => nft.type === NftType.SemiFungibleDCDT);
      const responseSFT = await axios.get(`${config.apiServiceUrl}/nfts/c?type=${NftType.SemiFungibleDCDT}`);
      expect(responseSFT.status).toBe(200);
      expect(responseSFT.data).toStrictEqual(nftsByTypeSFT.length);
    });
    ...
    it('should return the total number of nfts filtered by subType', async () => {
      const nftsBySubTypeNFT = nfts.data.filter((nft: any) => nft.subType === NftSubType.NonFungibleDCDTv2);
      const responseNFT = await axios.get(`${config.apiServiceUrl}/nfts/c?subType=${NftSubType.NonFungibleDCDTv2}`);
      expect(responseNFT.status).toBe(200);
      expect(responseNFT.data).toStrictEqual(nftsBySubTypeNFT.length);

      const nftsBySubTypeSFT = nfts.data.filter((nft: any) => nft.subType === NftSubType.SemiFungibleDCDT);
      const responseSFT = await axios.get(`${config.apiServiceUrl}/nfts/c?subType=${NftSubType.SemiFungibleDCDT}`);
      expect(responseSFT.status).toBe(200);
      expect(responseSFT.data).toStrictEqual(nftsBySubTypeSFT.length);
    });
  ...
  describe('GET /nfts/:identifier', () => {
    it('should return the nft details with owner field', async () => {
      const nfts = await axios.get(`${config.apiServiceUrl}/nfts?size=1&type=${NftType.NonFungibleDCDT}`);
    ...
    it('should return the nft details with supply field', async () => {
      const nfts = await axios.get(`${config.apiServiceUrl}/nfts?size=1&type=${NftType.SemiFungibleDCDT}`);
      const identifier = nfts.data[0].identifier;
