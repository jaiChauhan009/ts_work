...
describe('Give token lock unlock', () => {
  test('Normal', async () => {
    ...
    await tokenManager.setAccount({
      ...
      kvs: [...baseKvs, e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 1_000 }])],
    });
    ...
    assertAccount(kvs, {
      balance: 0n,
      kvs: [...baseKvs, e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 0 }])],
    });

    const otherUserKvs = await otherUser.getAccount();
    assertAccount(otherUserKvs, {
      kvs: [e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 1_000 }])],
    });
  });
  ...
  test('With flow limit', async () => {
    ...
    assertAccount(kvs, {
      ...
      kvs: [
        ...
        e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 500 }]),
      ],
    });

    let otherUserKvs = await otherUser.getAccount();
    assertAccount(otherUserKvs, {
      kvs: [e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 500 }])],
    });
    ...
    assertAccount(kvs, {
      ...
      kvs: [
        ...
        e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 0 }]),
      ],
    });
  ...
  test('With flow limit', async () => {
    ...
    await user
      .callContract({
        ...
        gasLimit: 20_000_000,
        funcArgs: [],
        dcdts: [{ id: TOKEN_IDENTIFIER, amount: 500 }],
      })
      .assertFail({ code: 4, message: 'Flow limit exceeded' });
    ...
    assertAccount(kvs, {
      ...
      kvs: [
        ...
        e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 1_000 }]),
      ],
    });
  });
...
test('Donate tokens', async () => {
  ...
  await user
    .callContract({
      ...
      gasLimit: 20_000_000,
      funcArgs: [],
      dcdts: [{ id: TOKEN_IDENTIFIER2, amount: 1_000 }],
    })
    .assertFail({ code: 4, message: 'Wrong token sent' });
