...
export class RabbitMqConsumer {
    ...
    constructor(
        ...
        private readonly wsDcdtTokenHandler: RabbitMQDcdtTokenHandlerService,
        ...
        const events: RawEvent[] = rawEvents?.events
            ?.filter(
                (rawEvent: { address: string; identifier: string }) =>
                    ...
                    rawEvent.identifier ===
                        TRANSACTION_EVENTS.DCDT_LOCAL_MINT ||
        ...
        for (const rawEvent of events) {
            ...
            switch (rawEvent.name) {
                ...
                case PROXY_EVENTS.COMPOUND_REWARDS_PROXY:
                    ...
                    break;
                case TRANSACTION_EVENTS.DCDT_LOCAL_MINT:
                    await this.wsDcdtTokenHandler.handleDcdtTokenEvent(
                        new DcdtLocalMintEvent(rawEvent),
                    );
                    break;
                case TRANSACTION_EVENTS.DCDT_LOCAL_BURN:
                    await this.wsDcdtTokenHandler.handleDcdtTokenEvent(
                        new DcdtLocalBurnEvent(rawEvent),
                    );
                    break;
