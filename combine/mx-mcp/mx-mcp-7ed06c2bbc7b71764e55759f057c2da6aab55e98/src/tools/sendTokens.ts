...
export async function sendTokens(
  ...
  if (tokenOfAccount.raw.type === "MetaDCDT") {
    if (!amount) {
      return {
        content: [
          {
            type: "text",
            text: "No token amount provided for Meta DCDT",
          },
        ],
      };
    }
...
export const sendTokensToolDescription =
  "Create a nft, sft or meta dcdt transfer transaction and send it. Will send the specified token using the wallet set in the env to the specified receiver.";
