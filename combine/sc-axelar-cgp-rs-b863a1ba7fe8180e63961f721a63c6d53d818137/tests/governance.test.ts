...
beforeEach(async () => {
  ...
  deployer = await world.createWallet({
    ...
    kvs: [e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 1_000, nonce: 1 }])],
  });
});
...
describe('Execute proposal', () => {
  ...
  test('Dcdt transfer', async () => {
    ...
    await deployer.callContract({
      ...
      funcName: 'executeProposal',
      funcArgs: [contract, callData, e.U(0)],
      dcdts: [{ id: TOKEN_IDENTIFIER, amount: 1_000, nonce: 1 }],
    });
  ...
  test('Upgrade gateway dcdt error', async () => {
    ...
    await deployer.callContract({
      ...
      funcName: 'executeProposal',
      funcArgs: [gateway, callData, e.U(0)],
      dcdts: [{ id: TOKEN_IDENTIFIER, amount: 500, nonce: 1 }],
    }); // async call actually fails
    ...
    assertAccount(kvs, {
      ...
      kvs: [
        ...
        e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 500, nonce: 1 }]), // dcdt still in contract
      ],
    });
    ...
    assertAccount(await deployer.getAccount(), {
      balance: 10_000_000_000n,
      kvs: [
        e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 1_000, nonce: 1 }]), // got dcdt back
      ],
    });
  });
  ...
  test('Withdraw refund token', async () => {
    ...
    assertAccount(await deployer.getAccount(), {
      balance: 10_000_000_000n,
      kvs: [
        e.kvs.Dcdts([{ id: TOKEN_IDENTIFIER, amount: 2_000, nonce: 1 }]), // got dcdt back
      ],
    });
    assertAccount(await contract.getAccount(), {
      balance: 1_000n,
