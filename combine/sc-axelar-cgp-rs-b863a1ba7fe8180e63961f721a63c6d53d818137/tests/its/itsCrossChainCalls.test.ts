...
test(
  ...
  async () => {
    ...
    const { computedTokenId, tokenIdentifier } = await deployNewDcdt();
...
test(
  ...
  async () => {
    ...
    const callContractLogData = await getCallContractDataFromDcdtAsync(hash);
...
describe('Execute interchain transfer with data', () => {
  ...
  const checkItsProxyTokens = async (fsProxyContract: FSContract, contractPayload: Buffer, computedTokenId: string) => {
    const { tokenIdentifier } = await deployNewDcdt();
    ...
    await user
      .callContract({
        ...
        dcdts: [{ id: tokenIdentifier, amount: 1_000 }],
      })
