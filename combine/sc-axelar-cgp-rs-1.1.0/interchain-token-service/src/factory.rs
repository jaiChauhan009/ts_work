...
use crate::proxy_its::{
    DCDT_PROPERTIES_DECIMALS_BUFFER_INDEX, DCDT_PROPERTIES_TOKEN_NAME_INDEX,
    DCDT_PROPERTIES_TOKEN_TYPE_INDEX,
};
    ...
    + executable::ExecutableModule
{
    ...
    fn deploy_remote_interchain_token_raw(
        ...
        let token_identifier = token_identifier.unwrap_dcdt();
    ...
    fn deploy_remote_token_callback(
        ...
        match result {
            ManagedAsyncCallResult::Ok(values) => {
                ...
                let token_name = vec.get(DCDT_PROPERTIES_TOKEN_NAME_INDEX).clone_value();
